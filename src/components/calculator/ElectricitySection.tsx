    ...
    <GlassCard
      ...
      headerAction={
        <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
          <span className="text-xs text-slate-400 font-medium hidden sm:inline">{t.electricity.kwhRate}:</span>
          <div className="w-28">
            <NumberInput
    ...
    >
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {electricalEquipment.map(item => {
            ...
            return (
              <div
                key={item.id}
                className="rounded-2xl bg-slate-950/50 border border-slate-800 p-3.5 flex flex-col justify-between"
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="text-xs font-bold text-slate-200 block">{item.name}</span>
                    <span className="text-[11px] text-amber-400 font-mono mt-0.5 inline-block">
                ...
                <div className="mt-3 pt-2 border-t border-slate-800/60 flex items-center justify-between text-xs">
                  <span className="text-slate-400 font-mono">{kwh.toFixed(3)} kWh</span>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <input
                      ...
                      className="w-full bg-slate-900 border border-slate-700/60 rounded-lg px-2 py-1 text-xs text-slate-100 font-semibold"
                    />
                    <button
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700/80 border border-slate-700 text-slate-300 text-xs font-semibold transition-all"
        >
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 p-3.5 rounded-2xl bg-amber-950/30 border border-amber-500/20">
          <div className="flex items-center gap-2 text-xs text-amber-300">
            ...
            <span>
              Toplam Enerji: <b className="text-white">{b.totalElectricityKwh} kWh</b> (kWh Başı: {formatCurrency(electricity.unitRatePerKwh, activeCurrency)})
            </span>
          </div>

          <div className="text-sm font-bold text-white flex items-center gap-2">
            <span className="text-slate-400 font-normal text-xs">{t.electricity.totalCost}:</span>
